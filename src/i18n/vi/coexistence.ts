// VI translations - coexistence namespace — đã rebrand sang Vit Engine
// Style note: keep raw English for awkward technical terms (per project memory).
export const coexistence = {
  // Hero — đổi ClaudeKit → Vit Engine, ck init → vit init
  'coexistence.hero.title': 'Vit Engine có làm hỏng setup Claude Code hiện tại của tôi không?',
  'coexistence.hero.tldr':
    'Trả lời ngắn: **không, mặc định không.** Vit Engine cài vào `./.claude/` của project hiện tại, preserve project config đã có, và merge chọn lọc `settings.json`. Command cần cẩn thận là `vit init --fresh`.',
  ...
  'coexistence.hero.q3.body': 'Còn nguyên với install thường. `--fresh` có thể xóa file Vit-tracked hoặc folder kit legacy.',
  'coexistence.hero.q4.title': 'Project đã có .claude/ thì sao?',
  'coexistence.hero.q4.body': '`vit init` thường scan `.claude/` trong project, protect custom files, và merge `.claude/settings.json`.',

  // Section A — Install scope (two-card layout)
  'coexistence.installScope.heading': 'Sau khi cài, Vit Engine ở đâu?',
  'coexistence.installScope.intro':
    'Mặc định, Vit Engine chỉ cài vào project bạn đang mở. Nó không thay đổi cài đặt Claude Code dùng chung trên máy bạn.',
  'coexistence.installScope.project.title': 'Project install (mặc định)',
  'coexistence.installScope.project.tag': 'Khuyên dùng',
  'coexistence.installScope.project.command': 'vit init',
  ...
  'coexistence.installScope.global.command': 'vit init --global',
  ...
  'coexistence.conflict.heading': 'Vit Engine tránh clobber setup của bạn thế nào',
  ...
  'coexistence.conflict.tracking.heading': 'Vit Engine biết file nào do nó cài thế nào',
  'coexistence.conflict.tracking.body':
    'Vit Engine ghi file đã cài vào `metadata.json` và track riêng hooks/MCP servers đã inject. Metadata này dùng cho selective update, uninstall, và phân tích `--fresh`.',
  ...
  'coexistence.conflict.example.ck': 'Vit-tracked — refresh được',

  // Section C — Fresh warning
  'coexistence.fresh.heading': 'Command nên backup trước khi chạy',
  'coexistence.fresh.intro':
    '`vit init --fresh` là full reset. Khi có `metadata.json` hiện tại, nó xóa file Vit-owned và Vit-modified, đồng thời preserve tracked user-created files. Nếu không có metadata dùng được, nó fallback sang xóa các kit folders: `commands/`, `agents/`, `skills/`, `rules/`, `hooks/`. Không có auto-backup.',
  'coexistence.fresh.dangerCard.title': 'Cái gì có thể bị xóa',
  'coexistence.fresh.dangerCard.body': 'Install legacy hoặc thiếu metadata có thể mất mọi thứ trong các folder này. Install hiện tại có tracking sẽ xóa Vit-owned/modified files trong đó.',
  ...
  'coexistence.fresh.tip':
    'Chỉ cần refresh kit content? Dùng `vit init --yes`. Chỉ dùng `vit update -y` khi muốn update CLI package; sau đó Vit Engine có thể tự refresh kit content cho project/global đã cài, và sẽ bỏ qua nếu content đã latest.',

  // Section D — Migrate
  'coexistence.migrate.heading': 'Đang chuyển sang agentic IDE khác?',
  'coexistence.migrate.intro':
    '`vit migrate` reconcile agents, commands, skills, config, rules, và hooks từ Claude Code sang target providers như Codex, Cursor, Gemini CLI, OpenCode, v.v. File nguồn Claude Code vẫn nằm nguyên.',
  'coexistence.migrate.notMigrate': 'Không phải cái bạn cần? `vit migrate` để **đổi tool**, không phải upgrade version.',
  ...
  'coexistence.recipes.intro': 'Ba recipe cụ thể để sống chung custom setup + Vit Engine.',
  ...
  'coexistence.recipe.customStatusline.desc':
    'Chạy `vit init` (không `--global`). Project install chỉ ghi vào `./.claude/`, nên global Claude Code files trong `~/.claude/` không nằm trong scope.',
  ...
  'coexistence.recipe.customHookMerge.desc':
    '`settings.json` được merge chọn lọc mặc định. Vit Engine thêm hoặc refresh tracked hooks, còn user-only hook entries được preserve.',
  ...
  'coexistence.faq.q1': 'Vit Engine có overwrite CLAUDE.md của tôi không?',
  'coexistence.faq.a1':
    '`vit init` thường preserve `CLAUDE.md` đã có vì file này nằm trong user config. Global mode có thể copy `~/.claude/CLAUDE.md` khi chưa có; với `--fresh`, global `CLAUDE.md` có thể bị replace.',
  'coexistence.faq.q2': 'Custom commands và skills của tôi sao?',
  'coexistence.faq.a2':
    '`vit init` thường scan custom `.claude` files và protect chúng. Ngoại lệ là `--fresh`: nó xóa Vit-owned/modified files, và install legacy thiếu metadata sẽ fallback sang xóa component directories.',
  'coexistence.faq.q3': 'Uninstall Vit Engine thế nào?',
  'coexistence.faq.a3':
    'Dùng `vit uninstall`. Command này hỗ trợ `--local`, `--global`, `--all`, `--dry-run`, `--yes`, và mặc định preserve user-created hoặc modified files. Chỉ dùng `--force-overwrite` khi thật sự muốn full removal.',
  'coexistence.faq.q4': 'Update Vit Engine an toàn thế nào?',
  'coexistence.faq.a4':
    '`vit init --yes` chỉ refresh kit content, không update CLI package. `vit update -y` update CLI package trước; sau đó có thể tự refresh kit content cho local/global install nếu content chưa latest. Tránh `--fresh` cho update thường.',
} as const;
