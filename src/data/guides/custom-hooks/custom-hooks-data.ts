...
export interface HookDef {
  ...
  lines: number;
  triggers: string[];
  /** Default new-install state, based on the stable Vit Engine docs/changelog. */
...
export const hookCategories: HookCategory[] = [
  {
    ...
    hooks: [
      ...
      },
      {
        ...
        descEn: 'Optional switch for usage-quota-cache-refresh. It only adds quota awareness when enabled in .claude/settings.json.',
        descVi: 'Công tắc tuỳ chọn cho usage-quota-cache-refresh. Chỉ thêm context quota khi được bật trong .claude/settings.json.',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    hooks: [
      ...
      },
      {
        ...
        kits: ['ek'],
        triggers: ['UserPromptSubmit'],
        descEn: 'Opt-in gate (gate.enabled=false by default). Hard-blocks ship/merge/pr/deploy/publish and soft-warns commit/finalize/release when the working tree carries a large unsimplified diff (400 LOC / 8 files / 200 LOC single-file). Bypass via env CK_SIMPLIFY_DISABLED=1 or .claude/settings.json hooks.simplify-gate=false.',
        descVi: 'Gate opt-in (mặc định gate.enabled=false). Hard-block ship/merge/pr/deploy/publish và soft-warn commit/finalize/release khi working tree có diff lớn chưa simplify (400 LOC / 8 file / 200 LOC một file). Bypass qua env CK_SIMPLIFY_DISABLED=1 hoặc .claude/settings.json hooks.simplify-gate=false.',
      },
      {
    ...
    hooks: [
      {
        ...
        descEn: 'Remind to invoke /vit:cook after Plan subagent completes. Outputs plan path for new sessions.',
        descVi: 'Nhắc chạy /vit:cook sau khi Plan subagent hoàn thành. Xuất plan path cho session mới.',
      },
      {
      ...
      },
      {
        ...
        descEn: 'Validate vit:fix/vit:cook review artifacts (context-snippets, verification, review-decision, risk-gate) before finalize and ship-like actions. Optional via .claude/settings.json; fail-open on crash.',
        descVi: 'Validate review artifacts của vit:fix/vit:cook (context-snippets, verification, review-decision, risk-gate) trước khi finalize và ship-like actions. Có thể bật trong .claude/settings.json; fail-open khi crash.',
      },
    ],
  },
];
